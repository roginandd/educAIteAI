...
export class SmartQuizService {
  ...
  async analyzePdfAndGeneratePreview(
    ...
    try {
      ...
      return analyzePdfAndGenerateSmartQuizPreviewOutputSchema.parse({
        extractedText,
  ...
  async generatePreview(input: GenerateSmartQuizItemsPreviewInput): Promise<NormalizedGenerateSmartQuizItemsPreviewOutput> {
    const parsedInput = generateSmartQuizItemsPreviewInputSchema.parse(input);
    const expectedDraftCount = parsedInput.generationOptions?.count ?? 10;
    const sourceGroundingContext = createSourceGroundingContext(parsedInput.notes);

    const generatedTrace = await this.structuredAgentRunnerService.runStructuredPromptWithTrace({
    ...
    try {
      ensureExactDraftCount(generatedTrace.data.drafts.length, expectedDraftCount);

      return normalizedGenerateSmartQuizItemsPreviewOutputSchema.parse({
        ...generatedTrace.data,
        drafts: generatedTrace.data.drafts.map((draft, index) =>
          normalizeDraftForV1Contract(draft, sourceGroundingContext, index)),
      });
    } catch (error) {
...
function normalizeRecommendedTypeOrder(
  ...
  return unique.length > 0 ? unique : ["Conceptual"];
}
    ...
    switch (draft.itemType) {
      ...
      case "Debugging":
        return {
          ...
          language: readStringValue(draft.language)
            ?? draft.technicalLanguage
            ?? "Java",
          visibleTestCases: normalizeVisibleTestCases(draft.visibleTestCases ?? normalizedValidationConfig.visibleTestCases) ?? [],
      ...
      case "Algorithm":
        return {
          ...
          itemType: "Algorithm",
          expectedAnswer: normalizedExpectedAnswer,
          functionSignature: readStringValue(draft.functionSignature)
          ...
          starterCodeByLanguage: normalizeStarterCodeByLanguage(
...
function normalizeValidationConfigForItemType(
  ...
  switch (itemType) {
    ...
    case "Algorithm": {
      ...
      ) ?? createFallbackFunctionSignature(draft.technicalLanguage);
...
function normalizeStarterCodeByLanguage(value: unknown, technicalLanguage: string): Record<string, string> | undefined {
  ...
  return {
    [language || "Java"]: createFallbackStarterCode(language || "Java"),
  };
}
...
function normalizeSupportedLanguages(value: unknown, technicalLanguage: string): string[] {
  ...
  return fallback ? [fallback] : ["Java"];
}

function normalizeVisibleTestCases(value: unknown): Array<{ input: string; expectedOutput: string }> | undefined {
  if (!Array.isArray(value)) {
    return undefined;
  }
  ...
  const normalized = value
    .map((entry) => {
      if (typeof entry === "string") {
        return parseStringVisibleTestCase(entry);
      }
    ...
    .filter((entry): entry is { input: string; expectedOutput: string } => entry !== null);

  return normalized.length > 0 ? normalized : undefined;
}

function parseStringVisibleTestCase(value: string): { input: string; expectedOutput: string } | null {
  ...
  for (const separator of arrowSeparators) {
    ...
    const input = trimmed.slice(0, index).trim();
    const expectedOutput = trimmed.slice(index + separator.length).trim();
    if (input && expectedOutput) {
      return { input, expectedOutput };
    }
  }
...
function ensureExactDraftCount(actualDraftCount: number, expectedDraftCount: number): void {
  if (actualDraftCount !== expectedDraftCount) {
    throw new AppError(
      `Smart quiz item generator returned ${actualDraftCount} drafts but ${expectedDraftCount} were requested.`,
      "SMART_QUIZ_DRAFT_COUNT_MISMATCH",
      502,
    );
  }
}
