...
export function createPdfExtractionAgent(model = env.GOOGLE_GENAI_MODEL): LlmAgent {
  return new LlmAgent({
...
export function createPdfExtractionRunner(model?: string): InMemoryRunner {
  return new InMemoryRunner({
    appName: env.GOOGLE_ADK_APP_NAME,
    agent: createPdfExtractionAgent(model),
  });
}
