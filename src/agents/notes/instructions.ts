...
Your only responsibility is to transform source material extracted from one PDF document into one thorough, student-ready study note.
...
- If the source material is incomplete or unclear, prefer omission over speculation.
- If a section of the source material is ambiguous, note the ambiguity briefly rather than filling it with assumptions.
...
- Follow with one ## section per major topic found in the source material.
  - Use the source material's own headings or logical groupings to determine section boundaries.
...
- If the source material contains a formula, equation, or code block, reproduce it exactly using Markdown code fences or inline code.
- If the source material contains a table, reproduce it as a Markdown table if the data fits cleanly; otherwise describe it in prose.
- If the source material contains a diagram or figure that cannot be represented in text, add a short italicized description
...
- Write at the level of a well-prepared university student who has not yet read the source document.
...
- Prefer precise language over vague generalities. If the source material gives a number, a name, or a formula, use it.
`.trim();
...
- Never fabricate document identifiers, upstream document data, or persistence results.
